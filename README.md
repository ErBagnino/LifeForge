...
| **Nutrition** (tab) | Dashboard with **current / target / % / remaining** for calories, protein, carbs, fat and water; **Add Food · Scan Food · Add Water** one tap away; meals grouped by **Breakfast · Morning Snack · Lunch · Afternoon Snack · Dinner · Night Snack**; tap any meal to edit or delete it. **ADD FOOD** is one short form: what did you eat → meal (pre-selected from the clock, windows in `src/config/meals.ts`) → time → calories and macros (no categories; 16 presets only fill the fields; `1,5` works with an Italian keyboard). **SCAN FOOD**: photo → Gemini estimate shown as **ranges** ("~350–450 kcal", "~150–200 g"), wider when confidence is low → quick questions about what it can't see → a small correction chat ("it was turkey", "the rice was 250 g", "I added a spoon of oil") with before → after → meal + time → **ADD TO TODAY** (the middle value is logged, everything editable). Meals from photos are labelled **AI**. Photos are not stored unless you turn on *Save Food Photos* (off by default). The Coach can also log food and water you tell it about (preview + APPLY). |
...
**Resets** (Settings → Data): today, this week, workout history, nutrition history, game progress and erase everything. Each shows exactly what is deleted and what is kept first; game progress and a full erase require typing a confirmation phrase. The Coach uses the same resets and confirmations.
